...
class __Mustache_9f07eed621ded7d4019496a96165158a extends Mustache_Template
{
...
';
        // 'title' section
        $value = $context->find('title');
        $buffer .= $this->section51887472a57a28df75a729d84d317b9a($context, $indent, $value);
        $buffer .= $indent . '
';
        // 'hascontrols' section
        $value = $context->find('hascontrols');
...
';
    ...
    private function section51887472a57a28df75a729d84d317b9a(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
            <h4 id="instance-{{blockinstanceid}}-header" class="card-title d-inline">{{{title}}}</h4>
        ';
            $result = call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
