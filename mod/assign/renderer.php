...
class mod_assign_renderer extends plugin_renderer_base {
    ...
    public function render_assign_submission_status(assign_submission_status $status) {
        ...
        $modinfo = get_fast_modinfo($status->courseid);
        $cm = $modinfo->get_cm($status->coursemoduleid);
        $instanceid = $cm->instance;
        $assign  = $DB->get_record('assign', array('id'=>$instanceid), '*');
        
        if($assign->isedublock){
            ...
            $data = [
                ...
                "block_library"=> $assign->block_library,
            ...
            $o .=  html_writer::link("http://edublock.dataviz.co.uk?token=".$encode,"Access Assignment", array("class"=> "btn btn-sm btn-primary","target" => "_blank")
                    );
        } else {
            $urlparams = array('id' => $status->coursemoduleid, 'action' => 'editsubmission');
                    $o .= $this->output->single_button(new moodle_url('/mod/assign/view.php', $urlparams),
                                                       get_string('addsubmission', 'assign'), 'post'); 
        }
