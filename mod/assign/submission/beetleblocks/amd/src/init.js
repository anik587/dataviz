...
export const init = (xmlProject, userid, attemptNumber, editable) => {
    ...
    if (snapIDE) {
        ...
        // Customize BeetleBlocks and hide Cloud options.
        customizeSnap(snapIDE);
    }
};

const getSnapFrame = (userid, attemptNumber) => {
    return document.getElementById('snap_beetleblocks-' + userid + '-' + attemptNumber);
};
...
const getXMLInput = () => {
    return $('input[name="beetleblocks_xmlproject"]')[0];
};
...
const customizeSnap = (snapIDE) => {
    // Adding BeetleBlocks embedded info to BeetleBlocks menu
    if (snapIDE.snapMenu) {
        snapIDE.sourceSnapMenu = snapIDE.snapMenu;
        Str.get_string('beetleblocks_embedded', 'assignsubmission_beetleblocks').then(str => {
