...
export const init = (xmlProject, userid, attemptNumber, editable) => {
    ...
    if (snapIDE) {
        ...
        // Customize Snap4Arduino and hide Cloud options.
        customizeSnap(snapIDE);
    }
};

const getSnapFrame = (userid, attemptNumber) => {
    return document.getElementById('snap_snap4arduino-' + userid + '-' + attemptNumber);
};
...
const getXMLInput = () => {
    return $('input[name="snap4arduino_xmlproject"]')[0];
};
...
const customizeSnap = (snapIDE) => {
    // Adding Snap4Arduino embedded info to Snap4Arduino menu
    if (snapIDE.snapMenu) {
        snapIDE.sourceSnapMenu = snapIDE.snapMenu;
        Str.get_string('snap4arduino_embedded', 'assignsubmission_snap4arduino').then(str => {
