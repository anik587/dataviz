...
/**
 * This file contains the definition for the library class for snap4arduino submission plugin.
 *
...
/**
 * Library class for snap4arduino submission plugin extending submission plugin base class.
 *
 * @package   assignsubmission_snap4arduino
 * @copyright 2020 Sara Arjona <sara@moodle.com> and Joan Guillén <jguille2@xtec.cat>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assign_submission_snap4arduino extends assign_submission_plugin {

    /**
     * Get the name of the snap4arduino submission plugin.
     * @return string
     */
    public function get_name(): string {
        return get_string('snap4arduino', 'assignsubmission_snap4arduino');
    }
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
        ...
        // Add the Snap4Arduino iframe.
        $mform->addElement('hidden', 'snap4arduino_xmlproject', $xmlproject);
        $mform->setType('snap4arduino_xmlproject', PARAM_RAW);

        $html = $this->get_view_snapframe($submission->userid, $submission->attemptnumber, true, null, '100%', '560px', true);
        $mform->addElement('header', 'snap4arduinoProject', get_string('snap4arduino_project', 'assignsubmission_snap4arduino'));
    ...
    public function save(stdClass $submission, stdClass $data): bool {
        ...
        $params = [
            ...
            'other' => array(
                'content' => trim($data->snap4arduino_xmlproject),
                'pathnamehashes' => [],
            )
        ];
        ...
        $event = \assignsubmission_snap4arduino\event\assessable_uploaded::create($params);
        $event->trigger();
        ...
        if ($snapsubmission) {
            ...
            $snapsubmission->snap4arduino_xmlproject = $data->snap4arduino_xmlproject;
            $status = $DB->update_record('assignsubmission_snap_s4a', $snapsubmission);
            // Trigger submission updated event.
            $params['objectid'] = $snapsubmission->id;
            $event = \assignsubmission_snap4arduino\event\submission_updated::create($params);
            $event->set_assign($this->assignment);
            $event->trigger();
        } else {
            // Create a new Snap4Arduino submission.
            ...
            $snapsubmission->id = $DB->insert_record('assignsubmission_snap_s4a', $snapsubmission);
            $status = $snapsubmission->id > 0;
            // Trigger submission created event.
            $params['objectid'] = $snapsubmission->id;
            $event = \assignsubmission_snap4arduino\event\submission_created::create($params);
            $event->set_assign($this->assignment);
            $event->trigger();
        }
    ...
    public function remove(stdClass $submission): bool {
        ...
        if ($submissionid) {
            $DB->delete_records('assignsubmission_snap_s4a', ['submission' => $submissionid]);
        }
    ...
    /**
     * Display Snap4Arduino result in the submission status table.
     *
     * @param stdClass $submission
    ...
    public function view_summary(stdClass $submission, & $showviewlink): string {
        ...
        $snapsubmission = $this->get_snap_submission($submission->id);
        if ($snapsubmission) {
            // Only shows the Snap4Arduino tutton and loads the Snap4Arduino source.
    ...
    public function get_files(stdClass $submission, stdClass $user): array {
        ...
        $snapsubmission = $this->get_snap_submission($submission->id);
        if ($snapsubmission) {
            $filename = get_string('snap4arduinofilename', 'assignsubmission_snap4arduino');
            $files[$filename] = [$snapsubmission->snap4arduino_xmlproject];
        }
    ...
    public function delete_instance(): bool {
        ...
        $DB->delete_records('assignsubmission_snap_s4a',
    ...
    public function is_empty(stdClass $submission): bool {
        ...
        return empty($snapsubmission->snap4arduino_xmlproject);
    }
    ...
    public function submission_is_empty(stdClass $data): bool {
        return empty($data->snap4arduino_xmlproject);
    }
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission): bool {
        ...
        // Copy the assignsubmission_snap4arduino record.
        ...
        if ($snapsubmission) {
            ...
            $DB->insert_record('assignsubmission_snap_s4a', $snapsubmission);
        }
        return true;
    }
    ...
    public function get_editor_fields(): array {
        return ['snap' => get_string('pluginname', 'assignsubmission_snap4arduino')];
    }
    ...
    public function get_editor_text($name, $submissionid): string {
        if ($name == 'snap4arduino') {
            $snapsubmission = $this->get_snap_submission($submissionid);
            if ($snapsubmission) {
                return $snapsubmission->snap4arduino_xmlproject;
            }
        }
    ...
    private function get_snap_submission(int $submissionid) {
        ...
        return $DB->get_record('assignsubmission_snap_s4a', ['submission' => $submissionid]);
    }

    /**
     * Return the Snap4Arduino project content with the plugin toolbar.
     *
     *
     ...
     * @return string the html to display the Snap4Arduino content.
     */
    private function get_view_snapframe(string $userid, string $attempt, bool $loaded = false, string $xmlproject = null,
        ...
        $template->snapurl = "$CFG->wwwroot/mod/assign/submission/snap4arduino/run/index.html";
        if ($xmlproject) {
            $template->snap4arduino_xmlproject = $xmlproject;
        }
        if (!$userid) {
            $userid = $USER->id;
        }
        ...
        $html = $OUTPUT->render_from_template('assignsubmission_snap4arduino/snapview', $template);

        return $html;
    }

    /**
     * Return the XML project from a Snap4Arduino submission.
     *
     * @param  stdClass $submission The submission
     * @return string The XML project of the Snap4Arduino content.
     */
    private function get_xmlproject(stdClass $submission): string {
        ...
        if ($submission) {
            $snapsubmission = $this->get_snap_submission($submission->id);
            if ($snapsubmission) {
                $xmlproject = $snapsubmission->snap4arduino_xmlproject;
            }
        }
    ...
    /**
     * Get if the assignment has attached any XML Snap4Arduino file. If exists, it will be used as template for new submissions,
     * instead of open an empty Snap4Arduino project.
     *
    ...
    private function get_xmlproject_template(): string {
        ...
        foreach ($fsfiles as $file) {
            if ($file->get_mimetype() == 'application/xml') {
                ...
                if (strpos($content, 'app="Snap4Arduino')) {
                    // This is a Snap4Arduino file. Use it as template (instead of displaying an empty project to the students).
                    return str_replace(array("&#xD;", "\r", "\n"), "", $content);
                }
            }
        }
