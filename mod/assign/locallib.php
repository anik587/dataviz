...
class assign {
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->requireallteammemberssubmit = $formdata->requireallteammemberssubmit;
         ...
         if($formdata->isedublock){
            $update->isedublock = $formdata->isedublock;
            $update->instruction = $formdata->instruction;
            $update->help = $formdata->help;
            $update->hints = $formdata->hints;
            $update->solution = $formdata->solution;
            $blockArr = array();
            foreach ($formdata->block_library as $block) {
                $blockArr[] = $block;
            }
            $update->block_library = implode(",",$blockArr);
     ...
     * @return bool
     */
    public function assign_save_submission(stdClass $data, & $notices, $userid,$tabhelp,$tabhint,$tabsolution,$time) {
        global $CFG, $USER, $DB;
        ...
        //EduBlock Code
        $userSub = $DB->get_record('assignsubmission_onlinetext', array('submission'=>$submission->id), 'id');
        if(!empty($userSub)){
            $textSubmission = new stdClass();
            $textSubmission->id = $userSub->id;
            $textSubmission->tabhelp = $tabhelp;
            $textSubmission->tabhint = $tabhint;
            $textSubmission->tabsolution = $tabsolution;
            $textSubmission->time = $time;
             
            $DB->update_record("assignsubmission_onlinetext", $textSubmission);
        }
        if ($instance->teamsubmission && !$instance->requireallteammemberssubmit) {
            $team = $this->get_submission_group_members($submission->groupid, true);
