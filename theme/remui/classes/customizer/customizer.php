...
class customizer {
     ...
     * @param string                       $parent    Parent name of setting
    ...
    public function process($css = '') {
        ...
        $css = $this->process_left_sidebar($css);
        return $css;
    }

    /**
     * Import customizer user to database.
     *
     * @return void
     */
    public function import_user_tour() {
        global $DB, $CFG;

        $tours = [
            'Edwiser RemUI Theme Customizer' => 'usertour_customizer',
            'Edwiser RemUI Theme Customizer Start' => 'usertour_customizer_start'
        ];

        foreach ($tours as $name => $file) {
            if (!$DB->record_exists('tool_usertours_tours', array('name' => $name))) {
                $content = file_get_contents($CFG->dirroot . '/theme/remui/db/' . $file . '.json');
                \tool_usertours\manager::import_tour_from_json($content);
            }
        }
    }
}
